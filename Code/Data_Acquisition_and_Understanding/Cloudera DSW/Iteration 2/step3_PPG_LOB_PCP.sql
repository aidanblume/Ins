...
as
select A.*, B.segment, B.lob, B.ppg, B.ppg_name
...
(
    select
        ...
        , row_number() over (partition by nat.case_id order by seg.segmtn) as rn
    from 
        (select * from nathalie.lob_step1) nat
        inner join
        (select * from nathalie.lob_step2) s2
       ...
       --/*** get LOB ***/
        ...
        on nat.eligibleorgid = trim(eo.eligibleorgid)
