...
as
select case_id, cin_no, adm_dt, dis_dt, provider, provider_type, source
from 
(
    select *, row_number() over (partition by cin_no, provider_type, to_date(adm_dt), to_date(dis_dt) order by source asc) as rn
    from 
    (
...
/*
CORRECT PROVIDER SO IT REFLECTS PROVID, and ADD SNF NAMES
*/
...
as
select case_id, cin_no, adm_dt, dis_dt, provider2 as provider, provider_type, source, fullname as postdischarge_SNFLTCSAname
from
(
    select *, row_number() over(partition by cin_no, provider_type, to_date(adm_dt), to_date(dis_dt) order by source2 asc) as rn
    from
    ( 
        --select names primarily from QNXT's provider table
        select A.*, B.provid as provider2, B.fullname, 1 as source2
        ...
        union
        select A.*, B.provid as provider2, B.fullname, 2 as source2
        from nathalie.tmp_raw_input as A
        left join plandata.provider as B
        ...
        union
        select A.*, B.provid as provider2, B.fullname, 3 as source2
        from nathalie.tmp_raw_input as A
        left join plandata.provider as B
        on A.provider = B.fedid
        ...
        and B.fullname is not null
        union
        select A.*, B.provid as provider2, B.fullname, 4 as source2
        from nathalie.tmp_raw_input as A
        left join plandata.provider as B
        on A.provider = B.fedid
        ...
        and B.fullname is not null
        union
        select A.*, B.provid as provider2, B.fullname, 5 as source2 -- there is some kind of match to a fullname
        from nathalie.tmp_raw_input as A
        left join plandata.provider as B
        on A.provider = B.fedid
        ...
        union
        select A.*, A.provider as provider2, A.provider as fullname, 6 as source2 -- there is no match to a fullname, but A.SNF may not be null
        from nathalie.tmp_raw_input as A
        where A.provider is not null
        union
        select A.*, null as provider2, null as fullname, 6 as source2 -- A.provider is null
        from nathalie.tmp_raw_input as A
...
(
    ...
    (
        ...
        from 
        (
            select *, concat(cin_no, provider, postdischarge_SNFLTCSAname, provider_type, cast(row_number() over (partition by cin_no, provider, provider_type order by adm_dt asc) as string)) as rnstart from nathalie.tmp_respaned_input    
        ) L   
        left join
        (
...
(
    ...
    from nathalie.prjrea_step8_snf as All_inp
    left join
        ...
        (
            ...
            (
                ...
                ( -- Select SNF Cases that were active during the 90 day pre-inpatient admission window
                    ...
                    from nathalie.prjrea_step8_snf as IP
