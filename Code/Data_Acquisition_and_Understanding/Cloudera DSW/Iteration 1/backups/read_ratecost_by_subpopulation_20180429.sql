...
drop table if exists nathalie.njb_complete_cases_7;
drop table if exists nathalie.njb_complete_cases_8;
...
from nathalie.njb_unique_cases_2;
...
select count(*)
        ...
        , sum(case when ModerateOrSevereLiverOrRenalDisease>0 then 1 else 0 end) as ModerateOrSevereLiverOrRenalDisease
        , sum(case when MetastaticSolidTumor>0 then 1 else 0 end) as MetastaticSolidTumor
from nathalie.njb_complete_cases_2;
...
select count(*)
        ...
        , sum(case when ModerateOrSevereLiverOrRenalDisease>0 then 1 else 0 end) as ModerateOrSevereLiverOrRenalDisease
        , sum(case when MetastaticSolidTumor>0 then 1 else 0 end) as MetastaticSolidTumor
from nathalie.njb_complete_cases_8;




--LOOP BACK 3 BLOCKS, REPEAT UNTIL RESULT IS complete_cases_8
...
inner join nathalie.njb_hedis_er_2 er on c.cin_no = er.cin_no and c.case_id = er.case_id and c.adm_dt = er.adm_dt and c.dis_dt = er.dis_dt
...
select * from nathalie.njb_hedis_compliant_er;
...
(
	SELECT 
        ...
        , B.ER_visits
        , B.ER_Days
    FROM NATHALIE.njb_A AS A LEFT JOIN nathalie.njb_hedis_compliant_er AS B ON A.rownumber = B.rownumber - 1 -- A is earlier than B; 1st row for A is nulls/ancient dates
) AS S
;
