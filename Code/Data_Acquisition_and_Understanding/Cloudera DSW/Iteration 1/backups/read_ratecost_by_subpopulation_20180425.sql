...
select count(*) from njb_unique_cases
/*
...
select code, comorbid_lace
    , case when comorbid_lace in ('PreviousMyocardialInfarction') then 1 else 0 end as PreviousMyocardialInfarction
    , case when comorbid_lace in ('CerebrovascularDisease') then 1 else 0 end as CerebrovascularDisease
    , case when comorbid_lace in ('PeripheralVascularDisease') then 1 else 0 end as PeripheralVascularDisease
    , case when comorbid_lace in ('DiabetesWithoutComplications') then 1 else 0 end as DiabetesWithoutComplications
    , case when comorbid_lace in ('CongestiveHeartFailure') then 1 else 0 end as CongestiveHeartFailure
    , case when comorbid_lace in ('DiabetesWithEndOrganDamage') then 1 else 0 end as DiabetesWithEndOrganDamage
    , case when comorbid_lace in ('ChronicPulmonaryDisease') then 1 else 0 end as ChronicPulmonaryDisease
    , case when comorbid_lace in ('MildLiverOrRenalDisease') then 1 else 0 end as MildLiverOrRenalDisease
    , case when comorbid_lace in ('AnyTumor') then 1 else 0 end as AnyTumor
    , case when comorbid_lace in ('Dementia') then 1 else 0 end as Dementia
    , case when comorbid_lace in ('ConnectiveTissueDisease') then 1 else 0 end as ConnectiveTissueDisease
    , case when comorbid_lace in ('AIDS') then 1 else 0 end as AIDS
    , case when comorbid_lace in ('ModerateOrSevereLiverOrRenalDisease') then 1 else 0 end as ModerateOrSevereLiverOrRenalDisease
    , case when comorbid_lace in ('MetastaticSolidTumor') then 1 else 0 end as MetastaticSolidTumor
...
(    
    ...
    ( -- DIS_DT_UPDATED // use stay_interval to compute new discharge date
        select 
            ...
            , (case when PreviousMyocardialInfarction > 1 then 1 else PreviousMyocardialInfarction end) as PreviousMyocardialInfarction
            , (case when CerebrovascularDisease > 1 then 1 else CerebrovascularDisease end) as CerebrovascularDisease
            , (case when PeripheralVascularDisease > 1 then 1 else PeripheralVascularDisease end) as PeripheralVascularDisease
            , (case when DiabetesWithoutComplications > 1 then 1 else DiabetesWithoutComplications end) as DiabetesWithoutComplications
            , (case when CongestiveHeartFailure > 1 then 1 else CongestiveHeartFailure end) as CongestiveHeartFailure
            , (case when DiabetesWithEndOrganDamage > 1 then 1 else DiabetesWithEndOrganDamage end) as DiabetesWithEndOrganDamage
            , (case when ChronicPulmonaryDisease > 1 then 1 else ChronicPulmonaryDisease end) as ChronicPulmonaryDisease
            , (case when MildLiverOrRenalDisease > 1 then 1 else MildLiverOrRenalDisease end) as MildLiverOrRenalDisease
            , (case when AnyTumor > 1 then 1 else AnyTumor end) as AnyTumor
            , (case when Dementia > 1 then 1 else Dementia end) as Dementia
            , (case when ConnectiveTissueDisease > 1 then 1 else ConnectiveTissueDisease end) as ConnectiveTissueDisease
            , (case when AIDS > 1 then 1 else AIDS end) as AIDS
            , (case when ModerateOrSevereLiverOrRenalDisease > 1 then 1 else ModerateOrSevereLiverOrRenalDisease end) as ModerateOrSevereLiverOrRenalDisease
            , (case when MetastaticSolidTumor > 1 then 1 else MetastaticSolidTumor end) as MetastaticSolidTumor
        from
        ( --'INTERVAL_ADDED' // add interval between 1st discharge date and 2nd admit date to create subquery called 'interval_added'
            select --select with priority QNXT > CLM > ENC and join with MEMMO data
                ...
                , FS.chemo as chemo
                , FS.rehab as rehab
                , FS.transplant as transplant
                , (FS.PreviousMyocardialInfarction + SS.PreviousMyocardialInfarction) as PreviousMyocardialInfarction
                , (FS.CerebrovascularDisease + SS.CerebrovascularDisease) as CerebrovascularDisease
                , (FS.PeripheralVascularDisease + SS.PeripheralVascularDisease) as PeripheralVascularDisease
                , (FS.DiabetesWithoutComplications + SS.DiabetesWithoutComplications) as DiabetesWithoutComplications
                , (FS.CongestiveHeartFailure + SS.CongestiveHeartFailure) as CongestiveHeartFailure
                , (FS.DiabetesWithEndOrganDamage + SS.DiabetesWithEndOrganDamage) as DiabetesWithEndOrganDamage
                , (FS.ChronicPulmonaryDisease + SS.ChronicPulmonaryDisease) as ChronicPulmonaryDisease
                , (FS.MildLiverOrRenalDisease + SS.MildLiverOrRenalDisease) as MildLiverOrRenalDisease
                , (FS.AnyTumor + SS.AnyTumor) as AnyTumor
                , (FS.Dementia + SS.Dementia) as Dementia
                , (FS.ConnectiveTissueDisease + SS.ConnectiveTissueDisease) as ConnectiveTissueDisease
                , (FS.AIDS + SS.AIDS) as AIDS
                , (FS.ModerateOrSevereLiverOrRenalDisease + SS.ModerateOrSevereLiverOrRenalDisease) as ModerateOrSevereLiverOrRenalDisease
                , (FS.MetastaticSolidTumor + SS.MetastaticSolidTumor) as MetastaticSolidTumor
            from
...
DROP TABLE if exists NJB_COMPLETE_CASES_1;
DROP TABLE if exists NJB_COMPLETE_CASES_2;
DROP TABLE if exists NJB_COMPLETE_CASES_3;
DROP TABLE if exists NJB_COMPLETE_CASES_4;
DROP TABLE if exists NJB_COMPLETE_CASES_5;
DROP TABLE if exists NJB_COMPLETE_CASES_6;
...
where chemo != 1 and rehab != 1 and transplant != 1
;
...
drop table if exists nathalie.njb_HEDIS_ER6M_1;

create table nathalie.njb_HEDIS_ER6M_1 as
select distinct cin_no, case_id, adm_dt, dis_dt, adddate(adm_dt, -183) as window_start, adddate(adm_dt, -1) as window_end
...
select *
from (select cin_no, case_id, cl_id, ER_dos_1, ER_dos_last, Days_ER, source_table, datediff(ER_dos_last, ER_dos_1) as ERDays
...
from (  select hdr.cin_no, hdr.case_id, hdr.cl_id, hdr.dos_1, hdr.admit_dt_clm, hdr.disch_dt_clm
            , hdr.admit_dt_clm as ER_dos_1
            , hdr.disch_dt_clm as ER_dos_last
            , datediff(hdr.disch_dt_clm, hdr.admit_dt_clm) as Days_ER
            , 1 as source_table
        from `hoap`.`qnxt_hdr_inpsnf` hdr
        where substr(hdr.type_bill,1,2) in ('11','12')
...
select w.cin_no, w.case_id, w.adm_dt
--        , sum(case when er.er_dos_1 is not null then 1 else 0 end) as ER_visits_0
--        , sum(case when er.er_dos_1 is not null and er.er_dos_last is not null then 1 else 0 end) as ER_visits_1
        , sum(case when datediff(w.adm_dt, er.er_dos_1) >= 1 and datediff(w.adm_dt, er.er_dos_1) <= 183 
                    and datediff(w.adm_dt, er.er_dos_last) >= 1 and datediff(w.adm_dt, er.er_dos_last) <= 183
                    then 1 else 0 end) as ER_visits
        , sum(case when datediff(w.adm_dt, er.er_dos_1) >= 1 and datediff(w.adm_dt, er.er_dos_1) <= 183 
                    and datediff(w.adm_dt, er.er_dos_last) >= 1 and datediff(w.adm_dt, er.er_dos_last) <= 183
                    then er.Days_ER else 0 end) as Days_ER
        , sum(case when datediff(w.adm_dt, er.er_dos_1) >= 1 and datediff(w.adm_dt, er.er_dos_1) <= 183 
                    and datediff(w.adm_dt, er.er_dos_last) >= 1 and datediff(w.adm_dt, er.er_dos_last) <= 183
                    then datediff(er.er_dos_last, er.er_dos_1) else 0 end) as ERDays
from nathalie.njb_HEDIS_compliant w
left join NATHALIE.QS_ER_DETAIL er on w.cin_no = er.cin_no
group by w.cin_no, w.case_id, w.adm_dt
;

-- Check results by members
select count(distinct cin_no) as members
--        , sum(er_visits_0) as er_visits_0, sum(er_visits_1) as er_visits_1
        , sum(er_visits) as er_visits, sum(days_er) as days_er, sum(erdays) as erdays
...
(
	SELECT 
        ...
        , B.PreviousMyocardialInfarction, B.CerebrovascularDisease, B.PeripheralVascularDisease, B.DiabetesWithoutComplications, B.CongestiveHeartFailure
        , B.DiabetesWithEndOrganDamage, B.ChronicPulmonaryDisease, B.MildLiverOrRenalDisease, B.AnyTumor, B.Dementia, B.ConnectiveTissueDisease, B.AIDS
        , B.ModerateOrSevereLiverOrRenalDisease, B.MetastaticSolidTumor
        , B.severity
        , B.aprdrg
        , B.dis_status
        , B.provider
...
(
	SELECT 
        ...
        , A.PreviousMyocardialInfarction, A.CerebrovascularDisease, A.PeripheralVascularDisease, A.DiabetesWithoutComplications, A.CongestiveHeartFailure
        , A.DiabetesWithEndOrganDamage, A.ChronicPulmonaryDisease, A.MildLiverOrRenalDisease, A.AnyTumor, A.Dementia, A.ConnectiveTissueDisease, A.AIDS
        , A.ModerateOrSevereLiverOrRenalDisease, A.MetastaticSolidTumor
        , A.severity
        , A.aprdrg
        , A.dis_status
        , A.provider
