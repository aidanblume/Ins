...
Description:
Data Source:
Output: NATHALIE.NJB_DEMOGRAPHICS_ADDED, NATHALIE.NJB_ANALYTIC_SET, NATHALIE.NJB_ANALYTIC_SET_LACE
Project: Readmission
...
/***
Title:              CaseID_ReadmissionFlag
Description:        Creates a table with "is/is not a readmission" flags for each undup'd, consolidated case_id between 2015 and 2017 [inclusive].  
                    Draws from QNXT, CLM and ENC [Nathalie to Chee: ENC data will fall away when you join with finance data]
Version Control:    https://dsghe.lacare.org/chlee/Analysis_SWAT/tree/master/Code/Modeling
Data Source:        HOAP.HOA is used to generate HDFS>>NATHALIE.njb_transfers_absorbed, a deduplicated set of admissions where 
                    for the same member, each admission is at least 24 hours apart from another. 
Output:             NATHALIE.caseID_ReadmissionFlag
                    Field description:
...
from (
-- Qing finds QNXT has no ER rev_cd, therefore QNXT cases will not be able to trace to individual claim line for ER identification
-- For QNXT, use header level information to derive ER later
...
-- Check duplications in njb_HEDIS_compliant table
