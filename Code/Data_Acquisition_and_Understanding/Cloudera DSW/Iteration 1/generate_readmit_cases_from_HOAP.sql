/***
Title:              generate_readmit_cases_from_HOAP
Description:        Generates a data set containing readmission flags and predictors drawn primarily from HOAP.HOA "case" tables. 
Version Control:    https://dsghe.lacare.org/nblume/Readmissions/tree/master/Code/Data_Acquisition_and_Understanding/Cloudera%20DSW/Iteration2/
Data Source:        HOAP.HOA QNXT, CLM and ENC case tables. 
Output:             NATHALIE.NJB_ANALYTIC_SET_LACE
                    Field description:
                        case_id: a string that identifies a member's hospital stays and that is generated by concatenating all HOA case_ids when these concerned the same member and were separated by < 24 hours
                        adm_dt: admit date for 1st case id in the concatenation above
                        dis_dt: discharge date for the last case_id in the concatenation above
                        LOS: length of stay for the entire (cancatenated) case_id
                        days_since_prior_discharge: number of days since the member was discharged from an eventual preceding hospital stay
                        prior_stay_case_id: a concatenated string that identifies the previous stay for that member
                        is_a_30d_readmit: readmission flag
                    NATHALIE.NJB_ANALYTIC_SET, 
***/
...
Purpose:    (1) To merge data from HOA's 3 case tables with priority QNXT>CLM>ENC, and
            (2) To join to it demographic info about the member from the MEMMO table.  

issue: how were inpatient cases defined?
- Per HOA dictionary: TYPE_BILL = ‘facilitycode’ || ‘billclasscodev’ || ‘frequencycode’
  --> Our implementation: where srv_cat = '01ip_a'; 
- Specific to ER code, the following implementation was used: substr(hdr.type_bill,1,2) in ('11','12').
- Per Mary (implementation not used): 
...
Purpose:    Add HEDIS value set flags to each case. 

Issues: (1) Contested issue about whether or not to dedup CMS's value set table. See below for resolution. 
        (2) HEDIS dummies creation works but may not be as concise as possible. Alternatives are: 1) group_concat(concat('"',code,'"'),','), or 2) find_in_set(<string>,<string list>)
*/

-- Extract and format the value sets that are relevant to readmission (HEDIS). 
--
-- Note on code change: Qing Sun (consultant @Centurylink) held the opinion that the hedic value set file from CMS needed to be 
-- deduplicated. Per Qing: "Dedup flatfile.hedis_readmission_value_sets_2016 based on priority provided below if the same code has multiple value_set_name
-- Need to ensure that each code is unique in this table to avoid double counting." Qing's deduping code is saved in directory
-- "Make Lookup Tables" in file "hedis_readmission_value_sets_2016_dedup". Nathalie Blume (@LA Care) held the opinion that CMS intended
-- for certain codes to beincluded in multiple value sets and that since these are only used to flag cases for inclusion or for exclusion, 
-- and are not treated as cummulative cases, there is no risk of double counting any single case. Therefore the deduping was removed. 
drop table nathalie.hedis_readmission_value_sets_2016_trsfm;

create table nathalie.hedis_readmission_value_sets_2016_trsfm as
select distinct code, value_set_name
        , case when value_set_name in ('Pregnancy', 'Perinatal Conditions'
                                        , 'Non-live Births', 'Premature Births' --added by njb beyond HEDIS set from 2016
                                        , 'Deliveries', 'Deliveries Infant Record' --added by njb beyond HEDIS set from 2016
                                        ) then 1 else 0 end as Pregnancy
        , case when value_set_name in ('Chemotherapy') then 1 else 0 end as Chemo
        , case when value_set_name in ('Rehabilitation'
                                        ,'AOD Rehab and Detox' --added by njb beyond HEDIS set from 2016
                                        ) then 1 else 0 end as Rehab
        , case when value_set_name in ('Bone Marrow Transplant','Kidney Transplant','Organ Transplant Other Than Kidney'
                                        ) then 1 else 0 end as Transplant
from flatfile.hedis_readmission_value_sets_2016
where code_system in ('ICD10CM', 'ICD10PCS')

-- Add flags to main table
...
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b1 on a.case_dx1=b1.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b2 on a.case_dx2=b2.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b3 on a.case_dx3=b3.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b4 on a.case_dx4=b4.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b5 on a.case_dx5=b5.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b6 on a.case_dx6=b6.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b7 on a.case_dx7=b7.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b8 on a.case_dx8=b8.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b9 on a.case_dx9=b9.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b10 on a.case_dx10=b10.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b11 on a.case_dx11=b11.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b12 on a.case_dx12=b12.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b13 on a.case_dx13=b13.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b14 on a.case_dx14=b14.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b15 on a.case_dx15=b15.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b16 on a.case_dx16=b16.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b17 on a.case_dx17=b17.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b18 on a.case_dx18=b18.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b19 on a.case_dx19=b19.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm b20 on a.case_dx20=b20.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p1 on a.case_pr1=p1.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p2 on a.case_pr2=p2.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p3 on a.case_pr3=p3.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p4 on a.case_pr4=p4.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p5 on a.case_pr5=p5.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p6 on a.case_pr6=p6.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p7 on a.case_pr7=p7.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p8 on a.case_pr8=p8.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p9 on a.case_pr9=p9.code
left join NATHALIE.hedis_readmission_value_sets_2016_trsfm p10 on a.case_pr10=p10.code
;
...
/*
COMORBIDITIES TABLE
brought in from flatfile.icd10_laceComorbidity_crosswalk
There is no duplications per ICDs, hence no need to dedup
*/
...
(    
    ...
    ( -- DIS_DT_UPDATED // use stay_interval to compute new discharge date
        ...
        ( --'INTERVAL_ADDED' // add interval between 1st discharge date and 2nd admit date to create subquery called 'interval_added'
            ...
            from
            NATHALIE.NJB_UNIQUE_CASES_3 as FS
            inner join
            NATHALIE.NJB_UNIQUE_CASES_3 as SS
...
select count(*)
        ...
        , sum(case when ModerateOrSevereLiverOrRenalDisease>0 then 1 else 0 end) as ModerateOrSevereLiverOrRenalDisease
        , sum(case when MetastaticSolidTumor>0 then 1 else 0 end) as MetastaticSolidTumor
from nathalie.njb_complete_cases_4;
...
select count(*)
        ...
        , sum(case when ModerateOrSevereLiverOrRenalDisease>0 then 1 else 0 end) as ModerateOrSevereLiverOrRenalDisease
        , sum(case when MetastaticSolidTumor>0 then 1 else 0 end) as MetastaticSolidTumor
from nathalie.njb_complete_cases_6;
...
/*
--LOOPED 3 TIMES TO GENERATE nathalie.njb_complete_cases_8
*/
...
-- [not done: Exclude index hospital stays where value_set_name include "Potentially Planned Procedures" without "Acute Condition"]
-- (no need since all non-acute cases already removed)
...
/*
Select all ER visits by cin_no [no time window]
(merge QNXT>CLM>ENC; remove null if possible as Null is sorted in lower priority; unique by admit date)
*/

drop table if exists NATHALIE.NJB_ER_DETAIL;

create table NATHALIE.NJB_ER_DETAIL as
select *
from 
(
    select cin_no, case_id, cl_id, ER_dos_1, ER_dos_last, source_table, datediff(ER_dos_last, ER_dos_1) as ERDays
    ...
    (  
        select  
            hdr.case_id
            , hdr.cin_no
            , hdr.cl_id  --tk too narrow if goal is to circumscribe an ER event rather than a claim?
            , hdr.dos_1
            , hdr.admit_dt_clm
            , hdr.disch_dt_clm
            , min(hdr.admit_dt_clm) as ER_dos_1
            -- Explanation for why hdr and detail tables are both used in generating discharge date below
            ...
            , max(
                case 
                    when hdr.disch_dt_clm is not null then hdr.disch_dt_clm 
                    else det.thru_dt 
                end) as ER_dos_last
            , 1 as source_table
        ...
        union
        select 
            hdr.cin_no, hdr.case_id, hdr.cl_id, hdr.dos_1, hdr.admit_dt_clm, hdr.disch_dt_clm
            , det.dos as ER_dos_1
            , case when det.thru_dt is not null then det.thru_dt else hdr.disch_dt_clm end as ER_dos_last
            , 3 as source_table
...
from NATHALIE.NJB_ER_DETAIL
group by source_table
order by source_table;

-- Add ER visits within 6-month to the index hospitalization (for LACE)
-- Note: ER visits with missing (null) thru_dt in DETAIL table (discharge date) will be ignored
drop table nathalie.njb_hedis_er;

create table nathalie.njb_hedis_er as
...
inner join nathalie.njb_hedis_er er on c.cin_no = er.cin_no and c.case_id = er.case_id and c.adm_dt = er.adm_dt and c.dis_dt = er.dis_dt
;

/*
-- Cases with missing case_id, to be researched
...
(
	SELECT 
        ...
        , B.ER_visits
        , B.ER_Days
    FROM NATHALIE.njb_A AS A LEFT JOIN nathalie.njb_hedis_compliant_er AS B ON A.rownumber = B.rownumber - 1 
...
(
    ...
    FROM NATHALIE.njb_A AS A LEFT JOIN NATHALIE.njb_labeled_as_readmits AS B ON A.rownumber = B.rownumber - 1 
-- A is earlier than B
) AS S
;

/*
--because you kept as main case data from table 'A', which had a dummy rownumber = 1, you need to shear it off now from table njb_labeled_outcomes. 
--select * from 
delete from
njb_labeled_outcomes
where adm_dt = '1900-01-01'
;
--AnalysisException: Impala does not support modifying a non-Kudu table: nathalie.njb_labeled_outcomes // 
*/
...
/*
HEDIS step 3: exclude hospital stays where the index admission date is the same as the index discharge date. 
! Note: this only applies to index admission, not to readmission.
Not implemented because predictions will be made before discharge
*/
...
DEATH
exclude members who died before discharge (not implemented because poorly populated fields)
*/
...
(
    select 
        ...
        , ER_Visits
        , ER_Days
    from nathalie.njb_labeled_outcomes as A left join encp.members as B on A.cin_no = B.cin_no
) as S
...
drop table if exists NJB_ANALYTIC_SET_LACE
;

CREATE TABLE NJB_ANALYTIC_SET_LACE
STORED AS PARQUET
AS
SELECT 
    cin_no
...
-- Check CCI proportion
select cci, count(*) from NJB_ANALYTIC_SET_LACE group by cci;



select er_visits from njb_analytic_set_lace
...
-- aprdrg: TK join with dictionary 
...
-- drop table if exists nathalie.njb_analytic_set;
-- drop table if exists nathalie.njb_analytic_set_lace;
drop table if exists nathalie.njb_demographics_added;
drop table if exists nathalie.njb_hedis;
drop table if exists nathalie.njb_hedis_er;
drop table if exists nathalie.njb_hedis_er_2;
...
drop table if exists nathalie.njb_hedis_compliant_er;
drop table if exists nathalie.njb_no_pregnancy;
drop table if exists nathalie.njb_tmp;
drop table if exists nathalie.njb2_analytic_set_lace;
drop table if exists nathalie.njb2_hedis_readmission_value_sets_2016_dedup;
drop table if exists nathalie.njb2_icd10_groups;
drop table if exists nathalie.readm_eda_nzvar;
drop table if exists nathalie.readm_eda_sumstat;
drop table if exists nathalie.caseid_readmissionflag;
drop table if exists nathalie.hedis_readmission_value_sets_2016_dedup;
drop table if exists nathalie.njb_1;
